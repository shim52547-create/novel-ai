...
function AntiAI() {
  ...
  const handleDeepAnalyze = async () => {
    ...
    setLoading('deep');
    try {
      const res = await fetch('http://localhost:4000/api/anti-ai/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text }),
      });
  ...
  const handleFix = async () => {
    ...
    setLoading('fix');
    try {
      const res = await fetch('http://localhost:4000/api/anti-ai/fix', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text,
  ...
  const handleSaveToChapter = async () => {
    ...
    try {
      await fetch(`http://localhost:4000/api/chapters/${selectedChapter.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
